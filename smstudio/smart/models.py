...
class Nav(models.Model):
    title = models.CharField(
        max_length=100, db_index=True, verbose_name="элемент навигации"
    )
    slug = models.SlugField(
        unique=True, max_length=100, db_index=True, verbose_name="URL"
    )
    ...
    annotations2 = models.TextField(
        blank=True, verbose_name="второе значение или ссылка на карту"
    )
...
class Services(models.Model):
    ...
    menu_image = models.ImageField(
        upload_to="images", verbose_name="Изображение в меню"
    )
    page_image = models.ImageField(
        upload_to="images", verbose_name="Изображение на странице"
    )
    ...
    gallery_link = models.CharField(
        blank=True, max_length=100, verbose_name="ссылка на фотографии"
    )
...
class Prices(models.Model):
    service = models.ForeignKey(
        Services, on_delete=models.PROTECT, verbose_name="Услуга"
    )
    title = models.CharField(max_length=100, verbose_name="Заголовок")
    slug = models.SlugField(
        unique=True, max_length=100, db_index=True, verbose_name="URL"
    ...
    extraLink = models.CharField(
        blank=True,
        max_length=100,
        verbose_name="Ссылка на дополнительную информацию(если " "есть)",
    )
    is_published = models.BooleanField(default=True, verbose_name="Публикация")
...
class Extra(models.Model):
    extraContent = models.ForeignKey(
        Prices, on_delete=models.PROTECT, verbose_name="Дополнительня информация о..."
    )
    extra_image = models.ImageField(
        upload_to="images", blank=True, verbose_name="Изображение если есть"
    )
    title = models.CharField(max_length=100, verbose_name="Заголовок")
    slug = models.SlugField(
        unique=True, max_length=100, db_index=True, verbose_name="URL"
...
class Gallery(models.Model):
    galleryLink = models.ForeignKey(
        Services, on_delete=models.PROTECT, verbose_name="Ссылка на услугу"
    )
    title = models.CharField(blank=True, max_length=100, verbose_name="Заголовок")
    gallery_image = models.ImageField(
        upload_to="images", blank=True, verbose_name="Фото"
    )
...
class PaperType(models.Model):
    ...
    class Meta:
        ...
        verbose_name_plural = "тип бумаги"
        ordering = ["id"]


class TonerConsumption(models.Model):
    title = models.CharField(max_length=100, verbose_name="расход тонера")
    price_plus = models.PositiveIntegerField(verbose_name="Плюс к цене в рублях")
    image = models.ImageField(
        upload_to="images", blank=True, verbose_name="Изображение"
    )
